...
public class ExcelReader  {
    ...
    public ExcelReader(String fileName) {
        ...
        if(file.exists()){
            ...
            throw new RuntimeException("Excel file not found in: " + fileName);
        }
                ...
                if (skipHeaderRow) {
                    ...
                    data = new String[rows][cols];
                        for (int i = 0; i < rows; i++) {
                            for (int j = 0; j < cols; j++) {
                                Cell cell = sheet.getRow(i).getCell(j);
                                String cellData = getData(cell).toString();  //Original Case(Convert all data to String)
                                data[i][j] = cellData;
                            }
                        }
                    }
        ...
        if (cell != null) {
            switch (cell.getCellTypeEnum()) {
                ...
                case BLANK:
                    result = "";
                    break;
                case BOOLEAN:   //This case wasn't there in the original file(I had to include it so that I can use it with POJO class)
